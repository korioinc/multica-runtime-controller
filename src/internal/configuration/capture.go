...
// FromSnapshots is the worker's payload proof. Object UID/owner checks belong
// to the controller API client; a projected filesystem does not expose that UID.
func FromSnapshots(refs []SnapshotRef, inputRoot string) (Bundle, error) {
	b := Bundle{SchemaVersion: 1, Groups: []Group{}}
	if err := ValidateRefs(refs); err != nil {
		return b, err
	}
	for _, ref := range refs {
		root, err := projection(filepath.Join(inputRoot, ref.SourceGroup))
		if err != nil {
			return b, err
		}
		g := Group{Name: ref.SourceGroup, Directories: append([]string{}, ref.Directories...), Files: []File{}}
		entries, err := os.ReadDir(root)
		if err != nil {
			return b, err
		}
		if len(entries) != len(ref.Mappings) {
			return b, errors.New("snapshot payload differs from its selected mapping")
		}
		for _, mapping := range ref.Mappings {
			raw, _, err := readFile(filepath.Join(root, mapping.Key))
			if err != nil {
				return b, err
			}
			g.Files = append(g.Files, File{Target: mapping.Target, Mode: mapping.Mode, SHA256: core.Digest(raw), Content: raw})
		}
		if err = g.Validate(); err != nil {
			return b, err
		}
		if GroupDigest(g) != ref.Digest {
			return b, errors.New("snapshot payload or mapping digest mismatch")
		}
		b.Groups = append(b.Groups, g)
	}
	b.Digest = Digest(b.Groups)
